...
const ASSIGNMENTS: SeedAssignment[] = [
  ...
  },
  {
    ...
    problems: [
      { topicSlug: "arguments-vs-nonarguments", prompt: "'We should leave now, because the storm is coming.' versus 'We left because the storm was coming.' For each one, decide whether the right response is to weigh whether the reason justifies the action or to ask whether it's the true cause of what already happened — and explain why the two sentences call for different responses.", correctAnswer: "The first argues for a conclusion (we should leave), offering the storm as a reason to act — you'd respond by assessing whether that reason justifies leaving. The second explains why an already-completed leaving happened — you'd respond by asking whether that's the real cause. One invites you to evaluate support for a decision; the other, the accuracy of an explanation.", explanation: "Telling an argument from an explanation determines whether you should test the support or the proposed cause." },
      ...
      { topicSlug: "deductive-vs-inductive", prompt: "A pollster surveys a representative sample and projects the election winner. A geometry teacher proves the angles of a triangle sum to 180°. For each one, decide whether the conclusion is locked in once you accept the reasons or only made probable — and explain what could make the merely-probable one wrong even if it's carried out perfectly.", correctAnswer: "The geometry proof is guaranteed — given the axioms, the conclusion can't be false. The poll is only probable: even a perfect representative sample can miss because people change their minds, turnout differs, or sampling luck runs against you. Generalizing from a sample to a whole population is inductive, so even a well-run poll can be wrong.", explanation: "Deductive proof guarantees its conclusion; inductive generalization stays fallible even when done correctly." },
